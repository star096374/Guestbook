<?php
    ...
    if (isset($_SESSION['account'])) {
        if ($row[0] == $_SESSION['account']) {
            $del=$mysqli->prepare("DELETE FROM `message` WHERE id = ?");
            $del2=$mysqli->prepare("DELETE FROM `reply` WHERE postid = ?");
            $del->bind_param('s', $id);
            $del2->bind_param('s', $id);
            if($del->execute() && $del2->execute()) 
            {       
                echo "<script>";
            ...
            echo "<script>";
            echo "alert('You are not the poster of this post.')";
            echo "</script>";
            echo "<script>";
            echo "location.href='index.php'";
            echo "</script>";
